      ...
      <div className="w-full max-w-md animate-slide-up relative z-10">
        <div className="bg-white/90 backdrop-blur-2xl rounded-[3rem] p-10 shadow-2xl border border-slate-100">
          <div className="text-center mb-10">
            ...
            <p className="text-slate-400 font-bold uppercase tracking-[0.2em] text-[10px] mt-3">Institutional Access Gateway</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                ...
                >
                  {showPassword ? (
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M3.98 8.223A10.477 10.477 0 001.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.773 3.162 10.065 7.498a10.523 10.523 0 01-4.293 5.774M6.228 6.228L3 3" /></svg>
                  ) : (
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.43 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /></svg>
                  )}
                </button>
              </div>
            </div>
            ...
            {error && (
              <div className="bg-rose-50 border border-rose-100 px-5 py-4 rounded-2xl text-[11px] font-bold text-rose-500 text-center animate-shake">
                {error}
              </div>
            )}
            ...
            {!isDev ? (
              <button
                ...
                className="w-full bg-slate-900 hover:bg-slate-800 text-white font-black py-5 rounded-2xl shadow-2xl transition-all transform active:scale-95 disabled:opacity-50 text-[11px] uppercase tracking-[0.3em]"
              >
                {isSubmitting ? 'Verifying Node...' : 'Establish Session'}
              </button>
            ) : (
              <div className="grid grid-cols-2 gap-3">
                <button type="button" onClick={() => handleBypass(UserRole.ADMIN)} disabled={isSubmitting} className="bg-rose-600 hover:bg-rose-500 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-white shadow-xl transition-all">Admin</button>
                <button type="button" onClick={() => handleBypass(UserRole.LECTURER)} disabled={isSubmitting} className="bg-violet-600 hover:bg-violet-500 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-white shadow-xl transition-all">Faculty</button>
                <button type="button" onClick={() => handleBypass(UserRole.STUDENT)} disabled={isSubmitting} className="bg-emerald-600 hover:bg-emerald-500 col-span-2 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-white shadow-xl transition-all">Student</button>
              </div>
            )}
          </form>
